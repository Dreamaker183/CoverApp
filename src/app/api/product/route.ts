...
export async function GET() {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Consider returning mock data as a fallback during prototyping even on real API errors:
      // return NextResponse.json(mockApiResponse);
    ...
    if (data.code !== 0 || !data.data) {
        ...
        // Optionally, fall back to mock data here as well:
        // return NextResponse.json(mockApiResponse);
    }
    ...
    // Optionally, fall back to mock data on any catch:
    // return NextResponse.json(mockApiResponse);
